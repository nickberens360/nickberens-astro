...
from backend.core.llm_chain import get_llm_instances  # noqa: E402

# Import our retrieval systems
from backend.core.unified_retriever import UnifiedRetriever  # noqa: E402
...
class RetrievalBenchmark:
    ...
    def __init__(self, embeddings: Any, llm: BaseLanguageModel):
        ...
        self.contextual_retriever = UnifiedRetriever(
            embeddings=embeddings, llm=llm, persist_dir="benchmark_contextual_chroma"
        )

        self.non_contextual_retriever = NonContextualRetriever(
            embeddings=embeddings, llm=llm, persist_dir="benchmark_non_contextual_chroma"
        )
    ...
    async def index_test_data(self, force_rebuild: bool = True) -> None:
        ...
        logger.info(
            f"Non-contextual: {non_ctx_files + non_ctx_pub_files} files, {non_ctx_chunks + non_ctx_pub_chunks} chunks"
        )
    ...
    def _evaluate_retrieval_quality(self, query: TestQuery, documents: List[Document]) -> Tuple[float, float, float]:
        ...
        for doc in documents:
            ...
            if "content_types" in doc.metadata:
                ...
                type_matches = sum(
                    1
                    for expected in query.expected_content_types
            ...
            keyword_matches = sum(1 for keyword in query.expected_keywords if keyword.lower() in content_lower)
            score += keyword_matches / len(query.expected_keywords) * 0.6

            relevance_scores.append(score)

        # Overall relevance (weighted by position)
        relevance_score = sum(score * (1.0 / (i + 1)) for i, score in enumerate(relevance_scores)) / sum(
            1.0 / (i + 1) for i in range(len(relevance_scores))
        )
        ...
        covered_keywords = sum(1 for keyword in query.expected_keywords if keyword.lower() in all_content)
    ...
    def _calculate_summary_stats(
        self, contextual_results: List[RetrievalResult], non_contextual_results: List[RetrievalResult]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "improvement": {
                ...
                "percent_relevance_improvement": (
                    ((avg(ctx_relevance) - avg(non_ctx_relevance)) / avg(non_ctx_relevance) * 100)
                    if avg(non_ctx_relevance) > 0
                    else 0
                ),
            },
        }
    ...
    def print_summary(self, results: BenchmarkResults) -> None:
        ...
        print(
            f"  • Relevance Improvement:     +{improvement['relevance_improvement']:.3f} ({improvement['percent_relevance_improvement']:.1f}%)"
        )
        ...
        for ctx_result, non_ctx_result in zip(results.contextual_results, results.non_contextual_results):
            ...
            print(
                f"{query_short:<50} {ctx_result.relevance_score:<12.3f} {non_ctx_result.relevance_score:<12.3f} {improvement:+.3f}"
            )
