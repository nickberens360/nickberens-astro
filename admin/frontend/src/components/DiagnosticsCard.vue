<template>
  <v-card class="mb-6" variant="tonal">
    <v-card-title class="d-flex align-center">
      <v-icon class="mr-2" :color="overallColor">$tune</v-icon>
      Diagnostics
      <v-spacer />
      <v-btn size="small" variant="text" @click="$router.push('/settings/features')">
        Open Feature Flags
      </v-btn>
    </v-card-title>
    <v-divider />
    <v-card-text>
      <div class="d-flex flex-wrap ga-4 align-center">
        <v-chip :color="configColor" variant="flat" size="small">
          Config: {{ envConfigured }}/{{ envTotal }} env-only
        </v-chip>
        <v-chip :color="validationColor" variant="flat" size="small">
          Validation: {{ validationStatus || 'unknown' }}
        </v-chip>
        <v-chip :color="criticalColor" variant="flat" size="small">
    ...
    <div v-if="missingEnvKeys?.length" class="mt-3">
      <div class="text-caption text-medium-emphasis mb-1">Missing env-only keys:</div>
      <div class="d-flex flex-wrap ga-2">
        ...
        <v-chip v-if="missingOverflowCount > 0" size="x-small" variant="text">
          +{{ missingOverflowCount }} more
        </v-chip>
      </div>
    </div>
    <div v-if="error" class="mt-3 text-error text-caption">
      {{ error }}
    </div>
    </v-card-text>
  </v-card>
