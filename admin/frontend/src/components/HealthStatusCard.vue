<template>
  <v-card class="mb-6" variant="tonal">
    <v-card-title class="d-flex align-center">
      <v-icon :color="apiStatusColor" class="mr-2">$dashboard</v-icon>
      System Health
      <v-spacer />
      <v-chip :color="apiStatusColor" size="small" class="mr-2" variant="flat">
        API: {{ apiStatusLabel }}
      </v-chip>
      <v-chip :color="dbStatusColor" size="small" variant="flat">
        DB: {{ dbStatusLabel }}
      </v-chip>
    </v-card-title>
    <v-divider />
    <v-card-text>
      <div class="d-flex flex-wrap align-center ga-4">
        <div class="d-flex align-center ga-2">
          <v-icon size="small" :color="apiStatusColor">$web</v-icon>
          <span class="text-caption">API Base: {{ apiBase }}</span>
        </div>
        <div class="d-flex align-center ga-2">
          <v-icon size="small" :color="dbStatusColor">$database</v-icon>
