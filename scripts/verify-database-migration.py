...
from backend.core.query_data_manager import QueryDataManager
...
def get_table_info(db_path: Path) -> Tuple[Dict[str, List[str]], Set[str]]:
    """Get table structure and indices from a database."""
...
def verify_database_schema(
    ...
    actual_tables, actual_indices = get_table_info(db_path)
...
def trigger_database_initialization():
    """Trigger database initialization by creating managers."""
...
def main():
    ...
    admin_db_path = Path("backend/logs/admin_monitoring.db")
    rag_db_path = Path("backend/logs/rag_monitoring.db")

    # Verify local databases
    admin_success = verify_database_schema(
        admin_db_path, EXPECTED_ADMIN_TABLES, EXPECTED_ADMIN_INDICES, "Admin", args.verbose
    )

    rag_success = verify_database_schema(rag_db_path, EXPECTED_RAG_TABLES, EXPECTED_RAG_INDICES, "RAG", args.verbose)
