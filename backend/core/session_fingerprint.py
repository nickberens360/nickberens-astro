    ...
    def store_session_fingerprint(self, session_id: str, fingerprint: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            admin_db_manager.record_security_event(
                "session_fingerprint_stored", session_id, "low", fingerprint_json, None, None
            )
        ...
        try:
            with admin_db_manager.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    WHERE event_type = 'session_fingerprint_stored'
                    AND identifier = ?
    ...
    def monitor_session_fingerprint(
        ...
        try:
            ...
            if validation_result["risk_level"] in ["medium", "high"]:
                admin_db_manager.record_security_event(
                    f"session_fingerprint_{validation_result['risk_level']}_risk",
                    username,
                    validation_result["risk_level"],
                    f"Session fingerprint validation: {validation_result['reason']}",
                    ip_address,
                    user_agent,
                )
