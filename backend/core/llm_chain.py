...
CLAUDE_MODEL = os.getenv("CLAUDE_MODEL", "claude-3-5-sonnet-20240620") # Use latest compatible model
...
def create_multi_vector_retriever(docs, embeddings):
    """
    Creates a multi-vector retriever with separate vector stores for each data source.
    This is a best practice to prevent context bleeding and improve retrieval accuracy.
    """
    vectorstores = {}
    retriever_infos = []
    ...
    # Create a Chroma vector store for each source
    for source, source_docs in docs_by_source.items():
        ...
        try:
            # Use an ephemeral client for in-memory storage
    ...
    # Define retriever infos for the router chain
    retriever_infos = [
        {
            "name": "resume",
            "description": "Good for answering questions about Nick's professional work experience, job history, roles, responsibilities, and technical skills.",
            "retriever": vectorstores["resume"].as_retriever(search_kwargs={"k": 8}) # <-- INCREASED FROM 5 to 8
        },
        {
            "name": "about",
            "description": "Good for answering questions about Nick's background, personal story, design philosophy, and general professional approach.",
            "retriever": vectorstores["about"].as_retriever(search_kwargs={"k": 5}) # Increased k for more context
        },
        {
            "name": "illustration",
            "description": "Good for answering questions about Nick's art, illustrations, characters, and creative work.",
            "retriever": vectorstores["illustration"].as_retriever(search_kwargs={"k": 5})
        }
    ]

    # Return a dictionary of retrievers for manual routing
    return {info["name"]: info["retriever"] for info in retriever_infos}


def get_llm_instances():
    """Initialize LLM instances with proper error handling."""
    llms = {}
    try:
...
def create_qa_chain(llm):
    """Creates the final question-answering chain with a more robust system prompt."""
    system_prompt = (
...
def route_query_to_retrievers(query: str, retrievers: Dict[str, BaseRetriever]) -> List[BaseRetriever]:
    """
    A simple rule-based router to select the right retriever(s) for a query.
    This version uses a set of strings (names) to avoid TypeError with unhashable objects.
    """
    query_lower = query.lower()
    selected_names = set()

    # Define keywords for each retriever
    ...
    # Default case: if no specific keywords, search resume and about
    if not selected_names:
        logger.info("No specific keywords found, routing to 'resume' and 'about' retrievers.")
        selected_names.update(["resume", "about"])

    # Build the final list of retriever objects from the unique names
    return [retrievers[name] for name in selected_names if name in retrievers]


def invoke_with_fallback(retrievers: Dict[str, BaseRetriever], chat_history: List[BaseMessage], user_input: str, preferred_model: Optional[str] = None) -> Tuple[str, str]:
    """
    Main function to process a query using the multi-vector RAG setup with LLM fallback.
    """
    if not retrievers:
    ...
    for llm_name, llm_instance in llm_order:
        if not llm_instance:
            ...
            continue
        try:
            logger.info(f"Attempting to use {llm_name.title()}...")

            # 4. Create the final QA chain and invoke it
            ...
            logger.warning(f"{llm_name.title()} rate limit reached: {e}. Trying fallback.")
            # Let the loop continue to the next LLM
        except Exception as e:
            logger.error(f"{llm_name.title()} error: {e}. Trying fallback.")
            # Let the loop continue to the next LLM

    # If all LLMs failed
    logger.error("All LLM attempts failed.")
    return (
        "I'm sorry, I'm currently experiencing technical difficulties. Please try again in a few minutes."
    ), "fallback"