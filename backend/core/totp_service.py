    ...
    def enable_2fa_for_user(self, user_id: int, username: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with admin_db_manager.get_connection() as conn:
                ...
                if existing:
                    ...
                    cursor.execute(
                        """
                        UPDATE user_2fa
                        SET secret = ?, backup_codes = ?, is_enabled = 0, created_at = ?
                        WHERE user_id = ?
                    """,
                        (secret, ",".join(backup_codes), datetime.now(), user_id),
                    )
                else:
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?, 0, ?)
                    """,
                        (user_id, secret, ",".join(backup_codes), datetime.now()),
                    )
            ...
            admin_db_manager.record_security_event(
                "2fa_setup_initiated", username, "low", "2FA setup initiated - not yet verified", None, None
            )
    ...
    def verify_and_enable_2fa(self, user_id: int, username: str, token: str) -> Dict[str, Any]:
        ...
        try:
            with admin_db_manager.get_connection() as conn:
                ...
                cursor.execute("SELECT secret, is_enabled FROM user_2fa WHERE user_id = ?", (user_id,))
                row = cursor.fetchone()
                ...
                # Enable 2FA
                cursor.execute(
                    "UPDATE user_2fa SET is_enabled = 1, verified_at = ? WHERE user_id = ?", (datetime.now(), user_id)
                )

                # Log successful setup
                admin_db_manager.record_security_event(
                    "2fa_enabled", username, "low", "2FA successfully enabled and verified", None, None
                )
    ...
    def verify_2fa_token(self, user_id: int, username: str, token: str, is_backup_code: bool = False) -> Dict[str, Any]:
        ...
        try:
            with admin_db_manager.get_connection() as conn:
                ...
                cursor.execute(
                    """
                    SELECT secret, backup_codes, is_enabled, used_backup_codes
                ...
                if is_backup_code:
                    ...
                    if token not in backup_codes:
                        admin_db_manager.record_security_event(
                            ...
                            username,
                            "medium",
                            f"Invalid backup code attempt: {token[:4]}****",
                            None,
                            None,
                        )
                    ...
                    if token in used_codes:
                        admin_db_manager.record_security_event(
                            ...
                            username,
                            "high",
                            f"Attempted reuse of backup code: {token[:4]}****",
                            None,
                            None,
                        )
                    ...
                    cursor.execute(
                        "UPDATE user_2fa SET used_backup_codes = ? WHERE user_id = ?", (",".join(used_codes), user_id)
                    )
                    ...
                    admin_db_manager.record_security_event(
                        ...
                        username,
                        "medium",
                        f"Backup code used for authentication: {token[:4]}****",
                        None,
                        None,
                    )
                    ...
                    if not self.verify_totp(secret, token):
                        admin_db_manager.record_security_event(
                            ...
                            username,
                            "medium",
                            f"Failed 2FA token verification: {token[:2]}****",
                            None,
                            None,
                        )
    ...
    def disable_2fa_for_user(self, user_id: int, username: str) -> Dict[str, Any]:
        ...
        try:
            with admin_db_manager.get_connection() as conn:
                ...
                cursor.execute("SELECT is_enabled FROM user_2fa WHERE user_id = ?", (user_id,))
                row = cursor.fetchone()
                ...
                # Log security event
                admin_db_manager.record_security_event(
                    "2fa_disabled", username, "medium", "2FA disabled for user account", None, None
                )
    ...
    def get_2fa_status(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            with admin_db_manager.get_connection() as conn:
                ...
                cursor.execute(
                    """
                    SELECT is_enabled, created_at, verified_at, backup_codes, used_backup_codes
                    FROM user_2fa WHERE user_id = ?
                """,
                    (user_id,),
                )
