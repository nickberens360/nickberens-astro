...
try:
    # Lazy import to avoid circulars during early app startup
    from .admin_database import admin_db_manager  # type: ignore

    ADMIN_DB_AVAILABLE = True
except Exception:  # pragma: no cover - defensive fallback in limited environments
...
def get_topic_taxonomy(force_reload: bool = False) -> Optional[Dict[str, Any]]:
    ...
    if ADMIN_DB_AVAILABLE:
        try:
            json_str = admin_db_manager.get_admin_setting("taxonomy_settings")
            if json_str:
